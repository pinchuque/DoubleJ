...
namespace DoubleJ.Oms.DataAccess
{
    public class OmsContext : DbContext, IOmsContext
    {
        ...
        public IDbSet<AnimalLabel> AnimalLabel { get; set; }
        ...
        protected override void OnModelCreating(DbModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Configurations.Add(new AnimalLabelMap());
            ...
            modelBuilder.Entity<Product>().HasRequired(x=>x.BagSizeEntity).WithMany(x => x.ProductsBags).HasForeignKey(x=>x.BagSizeId).WillCascadeOnDelete(false);
            modelBuilder.Entity<Product>().HasRequired(x => x.BoxSizeEntity).WithMany(x => x.ProductsBoxes).HasForeignKey(x => x.BoxSizeId).WillCascadeOnDelete(false);
            modelBuilder.Entity<CustomerProductData>().HasRequired(x => x.BagSizeEntity).WithMany(x => x.CustomerProductDataBags).HasForeignKey(x => x.BagSizeId).WillCascadeOnDelete(false);
            modelBuilder.Entity<CustomerProductData>().HasRequired(x => x.BoxSizeEntity).WithMany(x => x.CustomerProductDataBoxes).HasForeignKey(x => x.BoxSizeId).WillCascadeOnDelete(false);
        }
