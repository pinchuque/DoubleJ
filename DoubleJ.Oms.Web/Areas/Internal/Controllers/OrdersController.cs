...
namespace DoubleJ.Oms.Web.Areas.Internal.Controllers
{
    public class OrdersController : Controller
    {
        ...
        private void FillDetailTemp(int customerId, Order order)
        {
            ViewBag.Products = _orderDetailService.GetAllProducts(order);
            ViewBag.CustomerLocations = _orderDetailService.GetCustomerLocations(customerId);
            ViewBag.AnimalLabels = _animalLabelRepository.GetAll().Select(x=>new AnimalLabelsViewModel()
            {
                Id = x.Id,
                Name = x.Name,
        ...
        {
            var entry = _coldWeightEntryService.GetColdWeightEntryDetail(Id);
           
            var coldWeight = _coldWeightEntryRepository.GetByOrderId(Id);
            if (coldWeight != null && coldWeight.ColdWeightEntryDetails != null)
            {
                var details = coldWeight.ColdWeightEntryDetails.Where(x => coldWeight.Order
                    .OrderDetail.SelectMany(y => y.Label).Any(l => l.CreatedDate > x.CreatedDate));
            ...
            return PartialView("_AnimalTracking", entry);
        }
